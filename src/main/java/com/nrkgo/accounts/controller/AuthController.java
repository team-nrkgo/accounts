...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<UserSession>> login(@Valid @RequestBody LoginRequest request,
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse
                    .error("Email not verified. A new verification email has been sent to " + request.getEmail()));
        }
    }
    ...
    public void verifyUser(@RequestParam String token,
        ...
        try {
            ...
            if (user != null) {
                ...
                response.sendRedirect(frontendUrl + "/login?error=verification_failed");
            }
        } catch (Exception e) {
            response.sendRedirect(frontendUrl + "/login?error=verification_failed");
        }
    ...
    public ResponseEntity<ApiResponse<String>> forgotPassword(
            @Valid @RequestBody com.nrkgo.accounts.dto.ForgotPasswordRequest request) {
        userService.initiatePasswordReset(request.getEmail());
        return ResponseEntity.ok(ApiResponse
                .success("If an account exists with this email, a password reset link has been sent.", null));
    }

    @PostMapping("/reset-password")
    public ResponseEntity<ApiResponse<String>> resetPassword(
            @Valid @RequestBody com.nrkgo.accounts.dto.ResetPasswordRequest request) {
        userService.resetPassword(request.getToken(), request.getNewPassword());
        return ResponseEntity.ok(ApiResponse
                .success("Password has been reset successfully. Please login with your new password.", null));
    }
